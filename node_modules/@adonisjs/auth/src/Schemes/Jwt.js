...
class JwtScheme extends BaseTokenScheme {
  ...
  async generateForRefreshToken (refreshToken, jwtPayload, jwtOptions) {
    ...
    const User = use(this._config.model)
    const UserToken = use(this._config.token)

    const plainToken = await UserToken.findBy('token', this.Encryption.decrypt(refreshToken))
    if(plainToken){
      const user = await User.find(plainToken.user_id)
